...
describe('common/redux/hasAllSlots', () => {
    ...
    it('dispatches success action when hasAllSlots succeeds', () => {
        ...
        return store.dispatch(hasAllSlots(1))
            .then(() => {
    ...
    it('dispatches success action when hasAllSlots succeeds', () => {
        const store = mockStore({});
        return store.dispatch(hasAllSlots(2))
            .then(() => {
                const actions = store.getActions();
                console.log(actions);
                expect(actions[0]).toHaveProperty('type', COMMON_HAS_ALL_SLOTS_BEGIN);
    ...
    it('dispatches success action when hasAllSlots succeeds', () => {
        const store = mockStore({});
        return store.dispatch(hasAllSlots(3))
            .then(() => {
                const actions = store.getActions();
                console.log(actions);
                expect(actions[0]).toHaveProperty('type', COMMON_HAS_ALL_SLOTS_BEGIN);
    ...
    it('dispatches success action when hasAllSlots succeeds', () => {
        const store = mockStore({});
        return store.dispatch(hasAllSlots(4))
            .then(() => {
                const actions = store.getActions();
                console.log(actions);
                expect(actions[0]).toHaveProperty('type', COMMON_HAS_ALL_SLOTS_BEGIN);
    ...
    it('dispatches success action when hasAllSlots succeeds', () => {
        const store = mockStore({});
        return store.dispatch(hasAllSlots(5))
            .then(() => {
                const actions = store.getActions();
                console.log(actions);
                expect(actions[0]).toHaveProperty('type', COMMON_HAS_ALL_SLOTS_BEGIN);
    ...
    it('handles action type COMMON_HAS_ALL_SLOTS_FAILURE correctly', () => {
        ...
        expect(state).not.toBe(prevState); // should be immutable
        expect(state.hasAllSlotsPending).toBe(false);
        expect(state.hasAllSlotsError).toEqual(undefined);
    });
