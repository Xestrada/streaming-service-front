...
describe('common/redux/hasFreindRequest', () => {
    ...
    it('dispatches success action when hasFreindRequest succeeds', () => {
        ...
        return store.dispatch(hasFreindRequest(1, 5))
            .then(() => {
    ...
    it('dispatches failure action when hasFreindRequest fails with empty parameter', () => {
        const store = mockStore({});
    ...
    it('dispatches failure action when hasFreindRequest fails with no pending', () => {
        const store = mockStore({});

        return store.dispatch(hasFreindRequest(1, 1))
            .catch(() => {
                const actions = store.getActions();
                expect(actions[0]).toHaveProperty('type', COMMON_HAS_FREIND_REQUEST_BEGIN);
                expect(actions[1]).toHaveProperty('type', COMMON_HAS_FREIND_REQUEST_FAILURE);
    ...
    it('dispatches failure action when hasFreindRequest fails with string', () => {
        const store = mockStore({});
    ...
    it('dispatches failure action when hasFreindRequest fails with wrong id/fid', () => {
        const store = mockStore({});

        return store.dispatch(hasFreindRequest(99, 3))
            .catch(() => {
                const actions = store.getActions();
                expect(actions[0]).toHaveProperty('type', COMMON_HAS_FREIND_REQUEST_BEGIN);
                expect(actions[1]).toHaveProperty('type', COMMON_HAS_FREIND_REQUEST_FAILURE);
    ...
    it('handles action type COMMON_HAS_FREIND_REQUEST_FAILURE correctly', () => {
        ...
        expect(state).not.toBe(prevState); // should be immutable
        expect(state.hasFreindRequestPending).toBe(false);
        expect(state.hasFreindRequestError).toEqual(undefined);
    });
