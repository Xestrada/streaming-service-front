...
describe('common/redux/ratedMovies', () => {
    ...
    it('dispatches success action when ratedMovies succeeds with id:1', () => {
        const store = mockStore({});

        return store.dispatch(ratedMovies(1))
            .then(() => {
    ...
    it('dispatches success action when ratedMovies succeeds with id:2', () => {
        const store = mockStore({});

        return store.dispatch(ratedMovies(2))
            .then(() => {
                const actions = store.getActions();
                expect(actions[0]).toHaveProperty('type', COMMON_RATED_MOVIES_BEGIN);
                expect(actions[1]).toHaveProperty('type', COMMON_RATED_MOVIES_SUCCESS);
    ...
    it('dispatches failure action when ratedMovies fails with empty parameter', () => {
        const store = mockStore({});
    ...
    it('dispatches failure action when ratedMovies fails with negative id', () => {
        const store = mockStore({});
    ...
    it('dispatches failure action when ratedMovies fails with string', () => {
        const store = mockStore({});
    ...
    it('handles action type COMMON_RATED_MOVIES_FAILURE correctly', () => {
        ...
        expect(state).not.toBe(prevState); // should be immutable
        expect(state.ratedMoviesPending).toBe(false);
        expect(state.ratedMoviesError).toEqual(undefined);
    });
