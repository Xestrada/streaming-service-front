...
describe('common/redux/removeFriend', () => {
    ...
    it('dispatches failure action when removeFriend fails with empty parameter', () => {
    ...
    it('dispatches failure action when removeFriend fails with string', () => {
        const store = mockStore({});
    ...
    it('dispatches failure action when removeFriend fails with negative id', () => {
        const store = mockStore({});

        return store.dispatch(removeFriend(-1, -1))
            .then(() => {
                const actions = store.getActions();
                console.log(actions);
                expect(actions[0]).toHaveProperty('type', COMMON_REMOVE_FRIEND_BEGIN);
    ...
    it('dispatches failure action when removeFriend fails with single augment', () => {
        const store = mockStore({});

        return store.dispatch(removeFriend(1))
            .then(() => {
                const actions = store.getActions();
                console.log(actions);
                expect(actions[0]).toHaveProperty('type', COMMON_REMOVE_FRIEND_BEGIN);
    ...
    it('handles action type COMMON_REMOVE_FRIEND_FAILURE correctly', () => {
        ...
        expect(state).not.toBe(prevState); // should be immutable
        expect(state.removeFriendPending).toBe(false);
        expect(state.removeFriendError).toEqual(undefined);
    });
